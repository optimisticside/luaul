...
--[[
	...
	Note that this can be overriden by the user (since it's retrieved through
...
--[[
	Expects to read a certain type of token. If this token is not found, then
...
--[[
	Helper function to parse things separate by a delimiter token. This is used
	for expression lists, and other things like that.
]]
...
-- Note that the `if` that starts the if-else expression must be consumed prior
-- to calling ths function.
...
function Parser:parseTableConstructor()
	...
	while canContinue and not self:_accept(Token.Kind.RightBrace) do
		...
		elseif self:_peek(Token.Kind.Name) then
			-- If we see a name, it could either be the key of a value in the
			-- table, or just be a variable.
...
function Parser:parseFunctionArgs(selfParameter)
	...
	elseif self:_peek(Token.Kind.QuotedString) then
		...
		-- Since we've already checked for all other forms of providing
		-- function arguments, we can expect the user to provide normal
		-- function arguments with parentheses.
		self:_expect(Token.Kind.LeftParen)
